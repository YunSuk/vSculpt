...
public class DeformMeshJob : ThreadedJob
{
    ...
    protected override void ThreadFunction()
    {
        ...
        HashSet<int> subdiviedAffectedTriangleIndices = new HashSet<int>();
        foreach (int affectedTriangle in affectedTriangles)
        {
			List<int> temp = BreakDownTriangle (affectedTriangle);
			subdiviedAffectedTriangleIndices.UnionWith(temp);
        }

        // get points that should be modified and its value
		List<int> affectedVertexIndices = new List<int>();
		List<float> affectValues = new List<float> ();
		*/

		foreach (int index in affectedTriangles)//subdiviedAffectedTriangleIndices)
        {
			int i = meshTriangles [index];
			//int closestIndex = -1;
			//float closestSqrDist = float.MaxValue;
			Vector3 displace = new Vector3();
            for (int j = 0; j < contactPoints.Length; j++)
            {
				Vector3 diff =  meshPoints [i] - contactPoints [j];
				float sqrDist = diff.sqrMagnitude;

				// Too far away to give impact
				if (sqrDist > accuracySquare) 
				{
					continue;
				}
				/*
				if (Vector3.Dot (diff, contactNormals [j]) > 0) 
				{
					continue;
				}
				*/
				...
				*/
				//Vector3 diff = (contactPoints [j] - meshPoints [i]) ;
				if (Vector3.Dot (contactNormals [j], diff) > 0) {
					displace += diff / diff.sqrMagnitude;
				}
			...
			*/
			//Vector3 proj = Vector3.Project ((contactPoints [closestIndex] - meshPoints [i]).normalized * Math.Min(1,1.0f/(contactPoints [closestIndex] - meshPoints [i]).magnitude) , contactNormals [closestIndex]);
			//meshPoints[i] += proj * accuracy;
			meshPoints[i] += displace.normalized * accuracy;
        }
    ...
    private List<int> BreakDownTriangle(int triangleIndex)
    {
		...
		if ((vertexA - vertexB).sqrMagnitude < accuracySquare && (vertexA-vertexC).sqrMagnitude < accuracySquare 
			&& (vertexB-vertexC).sqrMagnitude < accuracySquare)
        {
            return newTriangleIndices;
        }
		...
		meshTriangles[triangleIndex + 1] = indexAB;
		meshTriangles[triangleIndex + 2] = indexAC;
